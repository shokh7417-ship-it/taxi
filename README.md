...
| `ENABLE_DRIVER_HTTP_LIVE_LOCATION` | **`true` / `1` recommended for Flutter / native driver apps**: **`POST /driver/location`** sets `last_live_location_at`, `live_location_active = 1` (same freshness rules as Telegram live), runs **`PulseDriverOnlineFromHTTP`** (`is_active=1`, then **`NotifyDriverOfPendingRequests`**). **Broadcast dispatch** for new rider requests also skips the extra **phone/car/plate** filters when this flag is on, so offers/`request_notifications` can be created for drivers who are live via HTTP before every profile field is filled. **Does not change** Telegram bot handler code. Default **off** keeps Telegram-only live semantics. Also **records offers in the DB if Telegram `Send` fails** so **`GET /driver/available-requests`** can poll. |
| `DISPATCH_DEBUG` | `true` / `1` — verbose dispatch / grid logs in **`MatchService`** |
| `DRIVER_AVAILABLE_REQUESTS_DEBUG` | `true` / `1` — when **`GET /driver/available-requests`** returns an **empty** queue, log driver snapshot + counters + computed reasons (see handler). |
| `DRIVER_AVAILABLE_REQUESTS_DEBUG_DRIVER_ID` | Optional internal **`users.id`** (same as **`X-Driver-Id`** when that header is the internal id). If `0` or unset, logs for **any** driver hitting the endpoint when debug is on. |
